...
import type { PlayerState, PlayerUpdate, GameStateSnapshot, BalanceUpdateEvent, PlayerRespawnEvent, LeaderboardUpdateEvent } from '../../../shared/multiplayer-types';
...
export class MultiplayerManager {
  ...
  public onBalanceUpdate?: (event: BalanceUpdateEvent) => void;
  public onPlayerRespawned?: (event: PlayerRespawnEvent) => void;
  public onLeaderboardUpdate?: (event: LeaderboardUpdateEvent) => void;
  ...
  private setupEventListeners() {
    ...
    // Player position update (lightweight, frequent)
    this.socket.on('playerUpdate', (update: PlayerUpdate) => {
      const player = this.otherPlayers.get(update.id);
      if (!player) return;
      
      // Safety check: Ensure update has required data
      if (!update.head || typeof update.head.x !== 'number' || typeof update.head.y !== 'number') {
        console.warn(`⚠️ Invalid playerUpdate data for ${update.id}, skipping`);
        return;
      }
    ...
    // Balance update event (for real-time money display)
    this.socket.on('balanceUpdate', (event: BalanceUpdateEvent) => {
      console.log(`💰 Balance update: Player ${event.playerId} now has $${event.newBalance.toFixed(2)} (${event.reason})`);
      
      // Update player's money in our local state
      const player = this.otherPlayers.get(event.playerId);
      if (player) {
        player.money = event.newBalance;
      }
    ...
    this.socket.on('playerRespawned', (event: PlayerRespawnEvent) => {
      console.log(`🔄 Player respawned: ${event.username} at (${event.newPosition.x.toFixed(1)}, ${event.newPosition.y.toFixed(1)}) with $${event.newBalance.toFixed(2)}`);
      
      // Update player position and balance
      const player = this.otherPlayers.get(event.playerId);
      if (player) {
        player.head = event.newPosition;
        player.money = event.newBalance;
        player.kills = 0; // Reset kills on respawn
        player.length = 10; // Reset length
    ...
    this.socket.on('leaderboardUpdate', (event: LeaderboardUpdateEvent) => {
      console.log(`🏆 Leaderboard update: ${event.leaderboard.length} players`);
      
      if (this.onLeaderboardUpdate) {
        this.onLeaderboardUpdate(event);
      }
    });
  }
   ...
   * Call this when your snake respawns after death
   */
  sendRespawnEvent() {
    this.socket.emit('playerRespawn');
    console.log(`🔄 Sent respawn event`);
  }
